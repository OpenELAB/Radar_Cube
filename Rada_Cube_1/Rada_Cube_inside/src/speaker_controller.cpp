...
void SpeakerController::setKeepOutputAlive(bool enabled)
{
    _keep_output_alive = enabled;
    if (!enabled) {
        _last_output_ms = millis();
    }
}

bool SpeakerController::keepOutputAlive() const
{
    return _keep_output_alive;
}
...
void SpeakerController::writeSilence(uint32_t sample_rate, uint16_t duration_ms)
{
    ...
    rememberOutputSample(0, 0);
}

void SpeakerController::writeFadeToSilence(uint32_t sample_rate, uint16_t duration_ms)
{
    if (!_i2s_started || sample_rate == 0 || duration_ms == 0) {
        return;
    }

    uint32_t total_frames = (sample_rate * duration_ms) / 1000U;
    if (total_frames == 0) {
        total_frames = 1;
    }
    uint32_t frame_index = 0;
    const size_t max_frames_per_write = sizeof(_stereo_buffer) / (2U * sizeof(int16_t));

    while (frame_index < total_frames) {
        const uint32_t frames_left = total_frames - frame_index;
        const size_t frames_this_write = static_cast<size_t>(
            frames_left < max_frames_per_write ? frames_left : max_frames_per_write);
        ...
        for (size_t i = 0; i < frames_this_write; i++) {
            ...
            const int16_t left = static_cast<int16_t>(
                (static_cast<int32_t>(_last_left_sample) * static_cast<int32_t>(frames_remaining)) /
                static_cast<int32_t>(total_frames));
            const int16_t right = static_cast<int16_t>(
                (static_cast<int32_t>(_last_right_sample) * static_cast<int32_t>(frames_remaining)) /
                static_cast<int32_t>(total_frames));
...
int16_t SpeakerController::applyFadeIn(int16_t sample)
{
    ...
    const uint32_t frames_done = _fade_in_frames_total - _fade_in_frames_left + 1U;
    const int16_t faded = static_cast<int16_t>(
        (static_cast<int32_t>(sample) * static_cast<int32_t>(frames_done)) /
        static_cast<int32_t>(_fade_in_frames_total));
...
void SpeakerController::shutdownAudioOutput()
{
    if (_i2s_started && _i2s_sample_rate != 0) {
        writeFadeToSilence(_i2s_sample_rate, SPEAKER_FADE_MS);
...
SpeakerController::PlaybackResult SpeakerController::playWav(AudioId audio, bool append_end_silence)
{
    ...
    if (_fade_in_next_playback) {
        _fade_in_frames_total = (wav_info.sample_rate * SPEAKER_FADE_MS) / 1000U;
        if (_fade_in_frames_total == 0) {
            _fade_in_frames_total = 1;
        }
...
void SpeakerController::writeAudioChunk(uint8_t* data, size_t bytes_read, const WavInfo& info)
{
    if (info.channels == 2) {
        ...
        for (size_t i = 0; i < frame_count; i++) {
            int16_t left = applyVolume(samples[i * 2]);
            int16_t right = applyVolume(samples[i * 2 + 1]);

            if (_fade_in_frames_left != 0 && _fade_in_frames_total != 0) {
                const uint32_t frames_done = _fade_in_frames_total - _fade_in_frames_left + 1U;
                left = static_cast<int16_t>(
                    (static_cast<int32_t>(left) * static_cast<int32_t>(frames_done)) /
                    static_cast<int32_t>(_fade_in_frames_total));
                right = static_cast<int16_t>(
                    (static_cast<int32_t>(right) * static_cast<int32_t>(frames_done)) /
                    static_cast<int32_t>(_fade_in_frames_total));
                _fade_in_frames_left--;
                if (_fade_in_frames_left == 0) {
                    _fade_in_frames_total = 0;
    ...
    for (size_t i = 0; i < sample_count; i++) {
        int16_t sample = applyVolume(mono_samples[i]);
