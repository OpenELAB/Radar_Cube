...
// 两键同时按下从机比较困难，我把config_mode 和 FACTORY_RESET_MODE 对换了
        ...
        for (int t = 0; t < WAKE_POLL_ROUNDS && !woke; t++) {
            ...
            if (Espnow.read(&msg)) {
                if (frame_validate(msg.data, msg.len, SLAVE_FRAME_HEAD, FRAME_WAKE_ACK)) {
                    // 这里判断一下是哪个从机返回的消息，只有两个从机都返回了ACK，唤醒任务才结束
...
static void handleMode(SysMode mode)
{
...
#ifdef INSIDE
        Matcher.clear_slave_mac(); // 清除从机 MAC
#endif

#ifdef OUTSIDE
        Matcher.clear_master_mac(); // 清除主机 MAC
    ...
    case PAIRED_MODE:
        ...
        // 改成配对过程中一直亮灯？还是加上系统开个任务做呼吸灯？，配对过程中一直灭灯指示不明显
